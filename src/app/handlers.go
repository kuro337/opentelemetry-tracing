...
func fibonacciHandler(deps *HandlerDependencies) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		_, span := deps.tracer.Start(r.Context(), "FIB_HANDLER")
		...
		if r.Method == http.MethodGet {
			...
			// Extract the number from the URL parameter.
			...
			_, span2 := deps.tracer.Start(r.Context(), "ReqGetFib")
			defer span2.End()

			// Call the Fibonacci function with the input number.
			result, err := Fibonacci(r.Context(), deps.tracer, uint(num))
			if err != nil {
